...
function getAddons($force_reload = false)
{
    ...
    while ($results && $row = $results->fetchRow()) {
        ...
        // only show addons which are readable by PHP and not protected
            if ($row['type'] == 'language' && file_exists($addon_folder.'.php')) {
                $addon_file = $addon_folder.'.php';
            } elseif (file_exists($addon_folder.$path_sep.'info.php')) {
                $addon_file = $addon_folder.$path_sep.'info.php';
            } elseif (file_exists($addon_folder.$path_sep.'info.ini')) {
                $addon_file = $addon_folder.$path_sep.'info.ini';
            } else {
                $addon_file = '';
            }
...
function getHumanReadableFileMakeTime($file)
{
        ...
        return @date($LANG['ADDON_FILE_EDITOR'][2]['DATE_FORMAT'], filemtime($file) + (int) TIMEZONE);
}
...
function getAddonInfos($addon_id_dir)
{
        ...
        if (is_numeric($addon_id_dir)) {
                ...
                if (method_exists($database, 'escapeString')) {
                        // with WB 2.8.3 SP2 the mysql_* routines were replaced by mysqli_ pendants for PHP 5.4/5.5 compatibility
                        $sql = "SELECT * FROM `$table` WHERE `directory` = '" . $database->escapeString(strip_tags($addon_id_dir)) . "'";
                } else {
                        // legacy: older WB 2.8.x releases still use mysql_ routines
